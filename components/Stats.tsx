      ...
      <div className="mx-auto max-w-[1480px] px-6 lg:px-12 py-16 lg:py-24">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-line border border-line">
          {STATS.map((stat, idx) => (
            <div
              key={stat.unit}
              className="bg-bg p-6 md:p-8 lg:p-10 group hover:bg-elev/40 transition-colors duration-700"
            >
              <div className="flex items-baseline justify-between font-mono text-[10px] tracking-[0.18em] uppercase text-mute mb-6 md:mb-8">
                <span>0{idx + 1}</span>
                <span>{stat.unit}</span>
              </div>

              <div className="font-display leading-[0.92] tracking-[-0.04em] text-ink">
                <span className="text-[clamp(2.6rem,7vw,5.6rem)]">
                  {stat.value}
                </span>
                <span className="text-[clamp(1.4rem,3vw,2.6rem)] text-ink-soft/80 align-top ml-1">
                  {stat.suffix}
                </span>
              </div>

              <div className="mt-5 flex items-center gap-3 font-mono text-[10px] tracking-[0.14em] uppercase text-ink-soft">
